import DepositModel, { DepositType, DepositDocument } from "./Deposit.model";
import { NotFoundError } from "../util/error/errors";
import FinancialRepoType from "./types/Financial.repo.type";

class FinancialRepo implements FinancialRepoType {
  async create(receipt: DepositType): Promise<DepositDocument> {
    try {
  ...
  async getMany(start: number, limit: number): Promise<DepositDocument[]> {
    try {
      return await DepositModel.find().skip(start).limit(limit);
    } catch (error) {
      throw error;
    }
  }
